...
class GraphController {
    ...
    def sync() {
        ...
        render text: "<html>Sync complete. Synced ${syncCount} users with ${linkCount} links at ${new Date()}</html>", contentType: "text/html"
    }
        ...
        if (params.id) {
            ...
            if (startingUser) {
                def resultsTable = startingUser.cypher(
                        "start myself=node({this}) MATCH myself-[:following]->friend-[:following]->fof WHERE fof.loginId <> myself.loginId RETURN myself, friend, fof")
